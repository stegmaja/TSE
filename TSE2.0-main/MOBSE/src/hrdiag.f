...
*       al. 2008).
*
*       Revised 22nd April 2020 by N. Giacobbo
*       the mass-loss (dmln) due to neutrinos during the SNe.
*       Reorganized the treatment of the rapid and delayed model.
*
      implicit none
*
      integer kw,kwp
...
* 
      ...
      real*8 mcmax,mcx,mcy,mcbagb,lambda1
...
*
      mass0 = mass
      if(mass0.gt.300.d0.and.saveflag.eqv..true.)then
         WRITE(99,*)' MASS EXCEEDED in hrdiag.f',mass0
         saveflag = .false.
      endif
*
      mt0 = mt
      if(mt0.gt.300.d0.and.saveflag.eqv..true.)then
...
*
            lambda1 = MIN(0.9d0,0.3d0+0.001d0*mass**5)
            tau = tscls(13)
            mcx = mcgbtf(tau,GB(2),GB,tscls(10),tscls(11),tscls(12))
            mcy = mc
            mc = mc - lambda1*(mcy-mcx)
            mcx = mc
            mcmax = MIN(mt,mcmax)   
         endif
         r = ragbf(mt,lum,zpars(2))
...
***
* Needed for the evaluation of the kick
* save the mfin before the SN
                  mfin = mt
*
***
                  if(nsflag.eq.0)then
...
*
                     ...
                     elseif(6.31d0.le.mc .and. mc.lt.6.75d0)then
                           mproto = 0.69d0*mc - 2.26d0
                     elseif(6.75d0.le.mc .and. mc.lt.7.6d0)then
                           mproto = 0.37d0*mc - 0.07d0
...
*
                     ffb = fallback(nsflag,mproto,mc,mt)
                     mrem = mproto + ffb*(mt - mproto)
*
...
*
                     ...
                     dmln = mrem - 6.6667d0*(sqrt(1.d0+0.3d0*mrem)-
     &                1.d0)
                     mt = mrem - min(dmln,0.5d0)
*                     
                     if(mt.le.mxns)then
...
***
            ...
            ECS = 1
* to mantain the bimodal distribution in fallback case
            ffb = 0.0d0
*
