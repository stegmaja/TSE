...
The csv table contains shows in each row the properties of a noteworthy event and the choices of initial conditions for the purpose of reproducability. The meaning of each column is specified in the function `store(...)` in the file `src/main.py` and read 
...
This row is apparently rich in information and can be broken into the following pieces. `t` and `status` describe the time and name of the event that occurred. The following entries of `y` contain the [integration variables](https://github.com/stegmaja/TSE/blob/main/README.md#integration-variables) at that time. This is followed by a range of stellar parameters `k1`, `k2`, ... , `radc3` describing the stellar types, masses, radii, epochs, spin rates, luminostites, envelope masses, envelope radii, core masses, and core radii. Lastly, the variables beginning with `ic.` recover the choice of initial conditions and systematic parameters of this run. They should read the same in each row of the csv file but might of course vary from file to file if the parameters were changed. This is helpful in case you want to rerun a particular system with a given choice of parameters. The exact meaning of each of the variables can be found in `src/initialconditions.py`.
