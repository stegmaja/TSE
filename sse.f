***
      subroutine sse(mass0,N,time,mv,rv,dmdtv,menvv,renvv,luminv,kwv,
     &     jj,tco,dms,Rmax,fb,z,tmax,kw,tRmax,mco0,dtp)
c-------------------------------------------------------------c
c
...
*
 ...
 30   j = j + 1
     ...
     &     .or.scm(j,2).gt.9)dmdtv(jj)=-1.e-12
        dmdtv(jj)=log10(abs(dmdtv(jj)))
        if(rv(jj).gt.Rmax)then
            tRmax=scm(j,1)*1.e6/ut
            Rmax=rv(jj)
        endif
...
*
      ...
      print*,'maximum stellar radius (AU) at (Myr)',Rmax,tRmax/(1.e6/ut)
