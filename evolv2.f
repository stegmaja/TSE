...
*
      ...
      PARAMETER(yeardy=365.d0,aursun=1.d0/0.00465d0)
      real*8 acc1,tiny
...
*
      ...
      real*8 bcm(1000000,34),bpp(80,10)
...
*
     ...
     &              /(2.d0*sep*sep*omv2)*0 !modified
               dmt(3-k) = MIN(dmt(3-k),0.8d0*dmr(k))*0 !modified
            else
               dmr(k) = 0.d0
               dmt(3-k) = 0.d0
            endif
...
*
     ...
     &                tc*(1.d0+q(3-k))**(5.d0/6.d0)
                  ! J.S.: Constant lag time
                  f = 0.014d0*12.4302d0*mass(k)/rad(k)/rad(k)/rad(k)*
     &               1.d0 ! lag time in seconds
                  tcqr = f*q(3-k)*raa6
                  rg2 = k2str(k)
...
*
         ...
         mass00(k) = m0
*
* Masses over 1000000Msun should probably not be trusted in the
* evolution formulae.
*
c         if(mt.gt.1000000.d0)then
c            WRITE(99,*)' MASS EXCEEDED ',mass1i,mass2i,tbi,ecci,mt
c            goto 140
c         endif
*
...
*
     ...
     &      or.tphys.lt.tiny)then
            ...
            teff1 = 10000000.d0*((1130.d0*lumin(1)/
     &                       (rad(1)**2.d0))**(1.d0/4.d0))
            ...
            teff2 = 10000000.d0*((1130.d0*lumin(2)/
...
*
         if(rad(j1).ge.1.002d0*rol(j1))then
            ...
            if(iter.eq.0) goto 7
            if(inttry) goto 7
            if(intpol.ge.1000000)then
c               WRITE(99,*)' INTPOL EXCEEDED ',mass1i,mass2i,tbi,ecci
               goto 140 
            endif
...
c         WRITE(99,*)' MAXIMUM ITER EXCEEDED ',mass1i,mass2i,tbi,ecci
         goto 140
      endif
      goto 5
*
...
* 
c      if(ospin(j1).gt.1.05d0*oorb)then
c        ospin(j1) = oorb
c        jspin(j1) = (k2str(j1)*rad(j1)*rad(j1)*(mass(j1)-massc(j1))+
c     &                k3*radc(j1)*radc(j1)*massc(j1))*ospin(j1)
c      endif
*
      iter = 0
      coel = .false.
...
*
            ...
            if(kst.gt.1000000) kst = kst - 1000000
            if(kst.eq.4)then
               aj(j2) = aj(j2)/tms(j2)
               massc(j2) = mass(j2)
            endif
...
*
         if(iter.le.10000000) dtm = MIN(dtm,dtmi(1),dtmi(2)) 
         dtm = MIN(dtm,tsave-tphys)
         dt = dtm*1.0d+06
         km = dt/tb
*
...
*         WRITE(99,*)' EVOLV2 ARRAY ERROR ',mass1i,mass2i,tbi,ecci
         ...
         open(61,file='result.dat',action='write',access='append')
         WRITE(61,*)' STOP: EVOLV2 ARRAY ERR ',mass1i,mass2i,tbi,ecci
         close(61)
         CALL exit(0)
         STOP
      elseif(jp.ge.40)then
         WRITE(*,*)' EVOLV2 ARRAY WARNING ',mass1i,mass2i,tbi,ecci,jp
      endif
      bcm(ip+1,1) = -1.0
      bpp(jp+1,1) = -1.0
*
